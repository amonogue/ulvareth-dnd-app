...
export default function UlvarethQuizApp() {
  ...
  // Send to DM: copy a URL the GM Admin can open to import
  function sendToDM() {
    const payload = {
      ts: new Date().toISOString(),
      player: player || "Player",
    ...
    // GitHub Pages vs local dev (single portal app)
    const isPages = typeof window !== "undefined" && /github\.io$/.test(window.location.host);
    const dmUrl = isPages
      ? `https://amonogue.github.io/ulvareth-dnd-app/#/gm/admin?data=${data}`
      : `http://localhost:5173/#/gm/admin?data=${data}`;

    navigator.clipboard?.writeText(dmUrl);
    alert("Link copied! Open GM Admin and paste in your browser.");
  }
