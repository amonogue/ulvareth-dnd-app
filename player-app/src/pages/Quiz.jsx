...
// Ulvareth Play Style Quiz — Player App (v2)
// Classic layout with a right sidebar. Name + Your Mix are pinned (sticky) while you scroll.
// CSV helper fixed ("\n" newlines) and BOM escaping. Includes runtime self‑tests.
// Chart: renders the original triangle (grid + layered triangles) with a filled indicator triangle.
...
export default function UlvarethQuizApp(){
  ...
  function resetSelections(){ setSelections({}); }

  const totals = useMemo(()=>{
    let SW=0, SE=0, SC=0;
    for (const q of QUESTIONS){
      const chosen = selections[q.id]||{};
      for (const opt of q.options){ if (chosen[opt.key]){ SW += opt.points.SW; SE += opt.points.SE; SC += opt.points.SC; } }
    }
    return { SW, SE, SC };
  }, [selections]);

  const { sw:nsw, se:nse, sc:nsc } = useMemo(()=> normalize(totals.SW, totals.SE, totals.SC), [totals]);
  const pct = useMemo(()=> ({ SW: Math.round(nsw*100), SE: Math.round(nse*100), SC: Math.round(nsc*100) }), [nsw,nse,nsc]);
  ...
  // Progress: how many questions have at least one option selected
  ...
  function buildCSV(){
    const rows = [
      { Player: player||"Player", Primary: primary.first.label, Secondary: primary.second.label, SW_total: totals.SW, SE_total: totals.SE, SC_total: totals.SC, SW_pct: pct.SW, SE_pct: pct.SE, SC_pct: pct.SC }
    ];
    return toCSV(rows);
  }
  ...
  function downloadCSV(){
    const csv = buildCSV();
    const safe = (player||"Player").trim().replace(/[^a-z0-9]+/ig, "_").replace(/^_+|_+$/g, "");
  ...
  async function copyCSV(){
    ...
    }catch(_){ alert('Could not copy. Try Download CSV.'); }
  }
  ...
  function saveResult(){
    const entry = {
      ...
      SW_total: totals.SW, SE_total: totals.SE, SC_total: totals.SC,
      SW_pct: pct.SW, SE_pct: pct.SE, SC_pct: pct.SC,
    };
    const next = [entry, ...saved].slice(0,50);
    setSaved(next);
    try{ localStorage.setItem('ulvareth_quiz_results', JSON.stringify(next)); }catch(_){ }
  }

  function clearSaved(){
    if(!confirm('Clear saved results?')) return;
    setSaved([]);
    try{ localStorage.removeItem('ulvareth_quiz_results'); }catch(_){ }
  }

  // --- Sidebar panels (both pinned) ---
  const ControlsPanel = (
    <div className="rounded-2xl border p-4 sticky top-2 z-20" style={{ borderColor: PALETTE.ash, background: '#fff' }}>
      <div className="grid gap-3">
        <label className="text-sm grid gap-1">
          <span>Name</span>
          <input value={player} onChange={(e)=> setPlayer(e.currentTarget.value)} placeholder="Your name" className="px-3 py-2 rounded-xl border" style={{ borderColor: PALETTE.ash, background: '#fff' }} />
        </label>
        ...
        <div className="grid gap-1 text-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="font-medium" style={{ color: PALETTE.aetherglass }}>{remaining} / {TOTAL_QUESTIONS}</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={downloadCSV} className="px-3 py-2 rounded-xl text-white" style={{ background: PALETTE.aetherglass }}>Download CSV</button>
          <button onClick={copyCSV} className="px-3 py-2 rounded-xl text-white" style={{ background: PALETTE.shroud }}>Copy CSV</button>
          <button onClick={saveResult} className="px-3 py-2 rounded-xl text-white" style={{ background: PALETTE.verdant }}>Save Result</button>
          <button onClick={resetSelections} className="px-3 py-2 rounded-xl text-white" style={{ background: PALETTE.ember }}>Reset</button>
        </div>
        <div className="text-sm grid gap-1">
          <span style={{ color: PALETTE.ash }}>Layout</span>
          <select value={layout} onChange={(e)=> setLayout(e.currentTarget.value)} className="px-2 py-2 rounded-lg border" style={{ borderColor: PALETTE.ash, background: '#fff' }}>
  ...
  const ResultsPanel = (
    <div className="rounded-2xl border p-4 sticky top-40 z-10" style={{ borderColor: PALETTE.ash, background: PALETTE.silver }}>
      <h4 className="font-semibold mb-2" style={{ color: PALETTE.aetherglass }}>Your Mix</h4>
      ...
      <div className="mt-3 text-sm">
        <div><b>Primary:</b> {primary.first.label}</div>
        <div><b>Secondary:</b> {primary.second.label}</div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen" style={{ background: PALETTE.pearl, color: PALETTE.coal }}>
      <div className="max-w-6xl mx-auto p-4 sm:p-6 grid gap-4 sm:gap-6">
        ...
        <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
          <div>
            <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight" style={{ color: PALETTE.aetherglass }}>Ulvareth Play Style Quiz</h1>
            <p className="text-sm" style={{ color: PALETTE.ash }}>Pick any number of answers per question. This is about play style, not correctness.</p>
          </div>
        </header>
        ...
        {layout === 'classic' ? (
          <div className="grid gap-4 sm:gap-6" style={{ gridTemplateColumns: 'minmax(0,1fr) 320px' }}>
            <div className="grid gap-3">
              {QUESTIONS.map((q)=>{
                ...
                return (
                  <div key={q.id} className="rounded-2xl border p-3 sm:p-4" style={{ borderColor: PALETTE.ash, background: answered? hexToRgba(PALETTE.silver, 0.6) : '#fff' }}>
                    <div className="mb-2 flex items-start justify-between gap-2">
                      <h3 className="font-semibold" style={{ color: PALETTE.aetherglass }}>{q.title}</h3>
                    </div>
                    <div className="grid gap-2">
                      {q.options.map((opt)=>{
                        ...
                        return (
                          <label key={key} className="flex items-start gap-3 rounded-2xl border p-3 cursor-pointer" style={{ borderColor: PALETTE.ash, background: checked? hexToRgba(PALETTE.pearl, .9) : hexToRgba(PALETTE.silver, .4) }}>
                            <input type="checkbox" className="mt-1 h-4 w-4" checked={checked} onChange={()=> toggle(q.id, opt.key)} style={{ accentColor: PALETTE.aetherglass }} />
                            <span className="leading-snug"><b className="opacity-80 mr-1 uppercase text-xs">{opt.key}</b> {opt.label}</span>
                          </label>
                        );
                      })}
                    </div>
          ...
          // Minimal layout: render controls + results below the questions
      ...
      {showTop && (
        <button onClick={()=> window.scrollTo({ top: 0, behavior: 'smooth' })} className="fixed bottom-4 right-4 px-3 py-2 rounded-xl text-white shadow-lg" style={{ background: PALETTE.aetherglass }}>
          Back to top
        </button>
      )}
    </div>
...
function Ternary(props){
  ...
  return (
    <svg viewBox={`0 0 ${S} ${S}`} width="100%" height={S}>
      ...
      {/* Outer triangle */}
      <polygon points={`${A.x},${A.y} ${B.x},${B.y} ${C.x},${C.y}`} fill={hexToRgba(PALETTE.silver, 0.25)} stroke={PALETTE.ash} strokeWidth={1.5} />
      ...
      {/* Axis labels */}
      <text x={A.x-36} y={A.y+18} fontSize="12" fill={PALETTE.coal}>Swords</text>
      <text x={B.x-42} y={B.y+18} fontSize="12" fill={PALETTE.coal}>Schemers</text>
      <text x={C.x-24} y={C.y-8} fontSize="12" fill={PALETTE.coal}>Seekers</text>
    </svg>
  );
}

// ----- Lightweight self‑tests (runtime, non‑blocking) -----
(function selfTests(){
  try {
    // toCSV basic
    ...
    // toCSV edge: quoting commas & quotes
    ...
    // toCSV multi-row & newline termination
    const csv3 = toCSV([{a:1,b:2},{a:3,b:4}]);
    console.assert(csv3.split('\n')[2].startsWith('3,4'), 'toCSV second row present');
    console.assert(csv3.endsWith('\n'), 'toCSV ends with a newline');

    // toCSV empty
    console.assert(toCSV([]) === '', 'toCSV handles empty input');

    // normalize zeros
    const n = normalize(0,0,0);
    console.assert(Math.abs(n.sw - 1/3) < 1e-9 && Math.abs(n.se - 1/3) < 1e-9 && Math.abs(n.sc - 1/3) < 1e-9, 'normalize handles zero totals');

    // normalize typical
    const n2 = normalize(2,1,1);
    console.assert(Math.abs(n2.sw - 0.5) < 1e-9 && Math.abs(n2.se - 0.25) < 1e-9 && Math.abs(n2.sc - 0.25) < 1e-9, 'normalize typical distribution');

    // ternary vertices
    const S = 100; const A = ternaryToXY(1,0,0,S), B = ternaryToXY(0,1,0,S), C = ternaryToXY(0,0,1,S);
    console.assert(A.x===0 && A.y===S && B.x===S && B.y===S && C.x===S/2 && C.y===0, 'ternaryToXY vertices');

    // ternary centroid (equal mix)
    const E = ternaryToXY(1/3,1/3,1/3,S);
    console.assert(Math.abs(E.x - S/2) < 1e-9 && Math.abs(E.y - (2*S/3)) < 1e-9, 'ternaryToXY centroid for equal mix');
  } catch (e) {
